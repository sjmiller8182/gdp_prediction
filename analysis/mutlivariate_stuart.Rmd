...
dat <- data %>% dplyr::select(-c(date))
dat <- dat %>% dplyr::select(-c(cpi))
```

## cpi - population collinearity

```{r}
summary(lm(population ~ cpi, data = data))
```
...
VARselect(dat, lag.max = 13, type = "both")
lsfit=VAR(dat,p=7,type="both")
summary(lsfit)
AIC(lsfit)
