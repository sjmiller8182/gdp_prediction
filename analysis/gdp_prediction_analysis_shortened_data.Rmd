...
(1-0.7391$B$)(1+0.3472$B$)($X_t$ - `r mean(data$gdp_change)`) = $a_t$ with $\sigma_a^2$ = `r est.s$avar`
...
tbl <- mdl_compare_var$remove_models(c('VARS AIC Both - R', 'VARS AIC HousePerm - R'))
```
...
The best hyperparameters based on this grid search are 25 repetitions and 4 hidden layers, which has a mean rolling window ASE of `r final.ase`.
